...
const SaveSystem = (() => {
  const profile = (typeof window !== 'undefined' && window.WEBCRAFT_PROFILE === 'personal') ? 'personal' : 'deployment';
  const WORLDS_KEY = CONFIG.SAVE_KEY + '_worlds_' + profile;
  ...
  function readStore() {
    ...
    const store = { version: SCHEMA_VERSION, profile, currentWorldId: null, worlds: {} };

    // Preserve and migrate the old single-world save without deleting it.
    try {
  ...
  function listWorlds() {
    return Object.values(readStore().worlds)
  ...
  function currentId() {
    return readStore().currentWorldId || null;
  }
  ...
  function createWorld(name, seed, mode) {
    ...
    const id = makeId();
    const finalSeed = seed != null && String(seed).trim() !== ''
      ? seed
  ...
  function clear(id) {
    ...
    if (target) return remove(target);
    return true;
  }

  return {
    profile, save, load, clear, listWorlds, currentId, getWorldMeta,
    createWorld, setCurrent, rename, remove
  };
})();
