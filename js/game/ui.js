...
function updateXP(xp,level,needed){
  let el=$('xp-hud');
  if(!el){
    el=document.createElement('div');el.id='xp-hud';
    el.innerHTML='<div class="xp-label"></div><div class="xp-track"><div class="xp-fill"></div></div>';
    const st=document.createElement('style');st.textContent='#xp-hud{position:fixed;left:50%;bottom:79px;transform:translateX(-50%);width:240px;z-index:6;pointer-events:none;text-align:center}.xp-label{font-size:11px;color:#9fe36f;text-shadow:0 1px 3px #000;margin-bottom:3px}.xp-track{height:6px;background:rgba(18,25,32,.88);border:1px solid rgba(119,165,89,.3);border-radius:99px;overflow:hidden}.xp-fill{height:100%;width:0;background:linear-gradient(90deg,#5bc735,#baf36c);box-shadow:0 0 9px rgba(120,225,76,.4)}@media(max-width:640px){#xp-hud{width:160px;bottom:61px}}';document.head.appendChild(st);document.body.appendChild(el);
  }
  const p=needed>0?Math.max(0,Math.min(1,xp/needed)):0;
  el.querySelector('.xp-label').textContent='Lv.'+level+' · XP '+xp+' / '+needed;
  el.querySelector('.xp-fill').style.width=(p*100).toFixed(1)+'%';
}
...
return{renderHotbar,setSelected,renderHeldItem,showItemName,showOverlay,showToast,showMiningProgress,hideMiningProgress,setHUD,setTime,updateXP,updateSurvival,showDeath,hideDeath,openPicker,closePicker,isPickerOpen,openInventory,openCraftingTable,closeInventory,isInventoryOpen,flashVignette};
})();